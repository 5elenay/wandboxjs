...
class WandBox {
     ...
     * @returns 
     */
    static compiler = async (lang, code) => {
        const langs = {
            cpp: {
                "compiler": "gcc-10.1.0",
                "options": "warning,boost-1.73.0-gcc-10.1.0,gnu++2a,cpp-no-pedantic"
            },
            "c++": {
                "compiler": "gcc-10.1.0",
                "options": "warning,boost-1.73.0-gccc-10.1.0,gnu++2a,cpp-no-pedantic"
            },
            c: {
        ...
        const compiled = await res.text().catch(e => e);
        const splitted = compiled.split("\n");
        const end = splitted.findIndex(i => i.includes("data: ExitCode"));
        const exitCode = splitted.find(x => x.includes("data: ExitCode")).replace("data: ExitCode:", "");
        const result = splitted.slice(5, end - 3).filter(i => (!i.includes("\r"))).join("\n").replace(/(data: |data:|StdOut:|StdErr:|\\n)/g, "").split("\n").filter(i => i !== "" && i !== "\r").join("\n");
        if (result.length) {
            return {
                result: result,
                exitCode: exitCode
